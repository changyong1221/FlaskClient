...
from utils.create_pic import save_to_pic_from_list, save_to_histogram_from_list, save_to_pie_from_list, save_compare_pic_from_vector
from utils.plt_config import PltConfig
# from src_scheduling.log import print_log


# 分析联邦学习场景下的平均任务处理时间
def analyze_federated_task_processing_time_results_single():
    # 1. settings
    data_path = "backup/results/global_processing_time.txt"
    
    data = pd.read_csv(data_path, header=None)
    data.columns = ['processing_time']
    print(len(data))
    data_list = data['processing_time'].to_list()
    count = (int)(len(data) / 10)
    show_list = []
    for i in range(count):
        show_list.append(data_list[i*10]) 
    
    # 2. 保存图片
    output_path = f"backup/pics/global_model_processing_time.png"
    ...
    save_to_pic_from_list(show_list, output_path, plt_config, show=False)
...
def analyze_federated_task_processing_time_results_comp():
    ...
    path = "backup/results/"
    path_list = [
            path + "global_processing_time.txt",
            path + "client_1_processing_time.txt"
        ]
    ...
    # 2. 保存图片
    output_path = f"backup/pics/federated_task_processing_time_comp_version.png"
    plt_config = PltConfig()
    plt_config.title = "task processing time in federated learning"
    plt_config.xlabel = "federated round"
    plt_config.ylabel = "task processing time"
...
# 分析联邦学习场景下的平均任务处理时间（算法对比）
def analyze_federated_task_processing_time_results_comp_global_vs_client():
    # 1. settings
    global_path = "backup/results/global_processing_time.txt"
    client_path = "backup/results/client_1_processing_time.txt"

    show_vector = []
    # 1. read global data
    global_data = pd.read_csv(global_path, header=None)
    global_data.columns = ['processing_time']
    print(len(global_data))
    global_data_list = global_data['processing_time'].to_list()
    global_data_trans = []
    for i in range(199):
        global_data_trans.append(global_data_list[i * 10])
    print(len(global_data_trans))
    show_vector.append(global_data_trans)
    
    # 1. read global data
    client_data = pd.read_csv(client_path, header=None)
    client_data.columns = ['processing_time']
    print(len(client_data))
    show_vector.append(client_data['processing_time'].to_list())

    # 2. 保存图片
    output_path = f"backup/pics/federated_task_processing_time_comp_version_2.png"
    plt_config = PltConfig()
    plt_config.title = "task processing time in federated learning"
    plt_config.xlabel = "federated round"
    plt_config.ylabel = "task processing time"
...
if __name__ == '__main__':
    ...
    analyze_federated_task_processing_time_results_single()
    # analyze_federated_task_processing_time_results_comp()
    # analyze_federated_task_processing_time_results_comp_global_vs_client()
